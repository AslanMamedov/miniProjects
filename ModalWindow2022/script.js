...
const bindResizeObserver = (modal) => {
	const contet = modal.querySelector(`.${CLASS_LIST.MODAL_DIALOG_BODY}`);

	const toggleShadows = () => {
		modal.classList.toggle(
			CLASS_LIST.MODAL_HAS_SCROLL,
			contet.scrollHeight > contet.clientHeight
		);
	}
	resizeObserver = new ResizeObserver(toggleShadows);
	resizeObserver.observe(contet);
};

const unbinResizeObserver = (modal) => {
	const contet = modal.querySelector(`.${CLASS_LIST.MODAL_DIALOG_BODY}`);

	resizeObserver.unobserve(contet);
	resizeObserver  = null;
	
}

document.getElementById('js-add-content-temp').addEventListener('click', () => {
